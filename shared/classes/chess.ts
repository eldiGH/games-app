...
export const getGameFromFENString = (fenString: string): ChessGame => {
	const pieces: Piece[] = [];
	...
	const whiteCastle: Castle = {
		kingSide: false,
		queenSide: false
	};
	const blackCastle: Castle = {
		kingSide: false,
		queenSide: false
	};
	...
	let enPassant: Point | undefined = undefined;
...
export class ChessGame {
	...
	constructor(
		...
		this.sortPieces();
		if (calculateMoves) this.calculateAllPossibleMoves();
	}
		...
		switch (piece.type) {
			case PieceType.PAWN: {
				...
				for (const diagonalPoint of diagonalPoints) {
					if (
						this.getPiece(diagonalPoint)?.color === oppositeColor ||
	...
	possibleMoves(piece: Piece) {
		...
		let myKingPosition =
			piece.type === PieceType.KING ? piece.position : this.findKingPosition(piece.color)?.position;

		if (!myKingPosition) return availableMoves;
	...
	canMoveTo(piecePoint: Point, newPoint: Point): boolean {
	...
	#forceMove(initialPosition: Point, newPosition: Point) {
		const piece = this.getPiece(initialPosition);
		if (!piece) return;

		const pieceToRemove = this.getPiece(newPosition);
		if (pieceToRemove) this.pieces = this.pieces.filter((piece) => piece !== pieceToRemove);
		...
		if (piece.type === PieceType.PAWN) {
			if (this.enPassant && this.enPassant.eq(newPosition)) {
				...
				else pawnToRemove = this.getPiece(new Point(newPosition.x, 4));

				if (pawnToRemove) this.pieces = this.pieces.filter((piece) => piece !== pawnToRemove);
			}
	...
	move(initialPosition: Point, newPosition: Point) {
		const piece = this.getPiece(initialPosition);
	...
	findKingPosition(color: PieceColor): Piece | undefined {
