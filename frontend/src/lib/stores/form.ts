...
export type FormReturn<Values extends Record<string, unknown>, MySchema> = {
	...
	submit: (onSubmit: (data: Values) => Promise<void>) => () => Promise<void>;
...
>(
			...
			? async () => {
					try {
						...
						for (const error of e.inner) {
							if (error.path) finalErrors[error.path] = error.errors[0];
						}
	...
	const submit = (onSubmit: (data: T) => Promise<void>) => async () => {
		let parsedValues: T = get(values);

		if (validate) parsedValues = await validate();
	...
	const handleBlur = (e: CustomEvent) => {
		...
		touched.update((touched) => {
			if (!(name.value in touched) && touched[name.value] === false) return touched;
